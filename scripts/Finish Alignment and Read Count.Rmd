...
per2_id <- "ENSGALG00010025047"

res <- results(dds,
               contrast = c("condition", "occludedEye", "openEye"),
               alpha    = 0.05)

pval <- res[per2_id, "pvalue"]
...
plotCounts(
  ...
  gene     = per2_id,
  intgroup = "condition",
  main     = paste0("Counts for PER2 (", per2_id, ")"),
  sub      = paste0("p = ", signif(pval, 3))
)
```
